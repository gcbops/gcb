(() => {
  ...
  var ChartModule = /* @__PURE__ */ (() => {
    ...
    function loadChart(type, animated = false, year = "all") {
      ...
      if (!func) {
        return AppUtils.showError(`No Apps Script function mapped for type ${type}`);
      }
      const cacheKey = func === "getYearlyChartData" ? `chartData_${type}_${year}` : `chartData_${type}`;
    ...
    function loadPrevYearCombinedChart() {
      ...
      if (!prevData) {
        fetchData("monthly_prev", prevCacheKey, (data) => {
          prevData = data;
          if (currentData) {
            drawChart(chartId, { prevYear: prevData, currentYear: currentData });
          }
      ...
      if (!currentData) {
        fetchData("monthly", currentCacheKey, (data) => {
          currentData = data;
          if (prevData) {
            drawChart(chartId, { prevYear: prevData, currentYear: currentData });
          }
      ...
      function fetchData(type, cacheKey, cb) {
        ...
        google.script.run.withSuccessHandler((data) => {
          if (!Array.isArray(data) || !data.length) {
            return AppUtils.showError(`${type} returned empty or invalid data`);
          }
    ...
    function drawChart(type, dataArray, debug = false, animated = false) {
      ...
      log("drawChart called \u{1F449}", { type, dataArray });
      const chartDiv = document.getElementById("chart_div_" + type);
      if (!chartDiv) {
          ...
          options: {
            ...
            plugins: {
              ...
              tooltip: {
                ...
                intersect: false,
                callbacks: {
                  label: (ctx2) => `${ctx2.dataset.label}: ${ctx2.formattedValue} hrs`
                }
              }
            },
      ...
      if (type === "hourly") {
        ...
        const labels2 = dataArray.map((r) => String(r[0] || ""));
        const values2 = dataArray.map((r) => Number(r[1]) || 0);
        ...
        chartInstances[type] = new Chart(ctx, {
          ...
          options: {
            ...
            plugins: {
              ...
              tooltip: {
                callbacks: {
                  label: (ctx2) => `${ctx2.formattedValue} hrs`
                }
              }
            },
            scales: {
      ...
      if (type === "monthly_prev_merged") {
        let fillMonthlyData = function(data) {
          return months.map((m) => {
    ...
    function drawRealtimeAnimatedChart() {
      ...
      chartDiv.innerHTML = `<canvas id="chartCanvas_${type}"></canvas>`;
      const ctx = document.getElementById(`chartCanvas_${type}`).getContext("2d");
      if (chartInstances[type]) {
        chartInstances[type].destroy();
        delete chartInstances[type];
      }
  ...
  var WidgetModule = /* @__PURE__ */ (() => {
    function loadTopCardMetrics() {
      AppUtils.cachedGScriptCall(
        ...
        (data) => {
          if (!data) {
            return AppUtils.showDashboardToast("No metrics found", "error");
          }
          const map = {
          ...
          document.querySelectorAll("#top-card-metrics .card").forEach((card) => {
            const heading = card.querySelector(".widget-heading")?.innerText.trim();
    ...
    function loadWorstProjectList() {
      ...
      const renderList = (data) => {
        listContainer.innerHTML = "";
        const sorted = [...data].sort((a, b) => (a[1] || 0) - (b[1] || 0)).slice(0, 10);
        for (const item of sorted) {
          const [project, hours, client] = item;
          const li = `
          <li class="list-group-item">
    ...
    function loadPercentagesDataCards(containerId, growthId, yearType) {
      ...
      if (!container.querySelectorAll(".widget-numbers").length && !container.querySelectorAll(".progress-bar").length) {
        return;
      }
      if (growthEl) {
      ...
      google.script.run.withSuccessHandler((data) => {
        ...
        const colors = yearType === "current" ? ["bg-arielle-smile", "bg-sunny-morning", "bg-ripe-malin", "bg-grow-early"] : ["bg-deep-blue", "bg-tempting-azure", "bg-ripe-malin", "bg-grow-early"];
    ...
    const loadCustomMonthlyReportsPageData = (callback = null) => loadReportsPageData(
      "monthlyReportsPageData",
      "getMonthlyReportsPageData",
      "monthlyReportsTable",
      callback
    );
    const loadCustomYearlyReportsPageData = (callback = null) => loadReportsPageData(
    ...
    function renderReportStatus(data) {
      ...
      $("#drive-storage-status").text(data.driveStorage ? "Connected" : "Not Connected").toggleClass("text-success", data.driveStorage).toggleClass("text-danger", !data.driveStorage);
      $("#discord-status").text(data.discord ? "Connected" : "Not Connected").toggleClass("text-success", data.discord).toggleClass("text-danger", !data.discord);
      $("#email-status").text(data.email ? "Connected" : "Not Connected").toggleClass("text-success", data.email).toggleClass("text-danger", !data.email);
    }
    function renderReportCounts(counts) {
      ...
      $("#automation-status").text(counts.automation ? "Active" : "Warning").toggleClass("text-success", counts.automation).toggleClass("text-danger", !counts.automation);
    }
  ...
  var allClientsPageModule = /* @__PURE__ */ (() => {
    let bound = false;
    const init = () => {
      if (bound) {
        return;
  ...
  var upsellOverviewPageModule = /* @__PURE__ */ (() => {
    let bound = false;
    const init = () => {
      if (bound) {
        return;
  ...
  var hourlyOverviewPageModule = /* @__PURE__ */ (() => {
    let bound = false;
    const init = () => {
      if (bound) {
        return;
  ...
  var reportsExportDataPageModule = /* @__PURE__ */ (() => {
    let bound = false;
    const init = () => {
      if (bound) {
        return;
  ...
  // src/pages/reports-monthly-report.js
  var reportsMonthlyReportModule = /* @__PURE__ */ (() => {
    let bound = false;
    ...
    function setGenerateState(btn, disabled) {
      ...
      $("#monthly-report-month").prop("disabled", disabled);
      $("#monthly-report-year").prop("disabled", disabled);
    }
      ...
      google.script.run.withFailureHandler((err) => {
        ...
        AppUtils.openModal("#app-modal", {
          ...
          footer: `
                ...
                `,
          onOpen($modal) {
            $modal.off(".monthlyReportModal").on("click.monthlyReportModal", ".btn-cancel", () => {
  ...
  // src/pages/reports-annual-report.js
  var reportsAnnualReportModule = /* @__PURE__ */ (() => {
    let bound = false;
    const REPORT_MAX_ATTEMPTS = 60;
    const REPORT_CHECK_INTERVAL = 5e3;
    const init = () => {
    ...
    function setGenerateState(btn, disabled) {
      btn.prop("disabled", disabled);
      $("#yearly-report-year").prop("disabled", disabled);
    }
      ...
      google.script.run.withFailureHandler((err) => {
        ...
        AppUtils.openModal("#app-modal", {
          ...
          footer: `
                    ...
                    `,
          onOpen($modal) {
            $modal.off(".annualReportModal").on(
